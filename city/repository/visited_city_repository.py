...
class VisitedCityRepository(AbstractVisitedCityRepository):
    def get_average_rating(self, city: City) -> float:
        ...
        not_rounded_average_rating = float(
            VisitedCity.objects.filter(city=city).aggregate(Avg('rating'))['rating__avg'] or 0
        )
    ...
    def count_user_visits(self, city: City, user: AbstractBaseUser) -> int:
        ...
        """
        return VisitedCity.objects.filter(city=city, user=user).count()

    def count_all_visits(self, city: City) -> int:
        """
        Возвращает количество посещений города city всеми пользователями.
        """
        return VisitedCity.objects.filter(city=city).count()
    ...
    def get_popular_months(self, city: City) -> list[int]:
        ...
        """
        return list(
            VisitedCity.objects.filter(city=city, date_of_visit__isnull=False)
    ...
    def get_user_visits(self, city: City, user: AbstractBaseUser) -> Sequence[dict]:
        ...
        """
        return list(
            VisitedCity.objects.filter(user=user, city=city)
            .select_related('city')
