...
class TestDjangoSubscribeRepository:
    def test_is_subscribed_true(self, mocker, repo):
        mock_qs = mocker.Mock()
        mock_qs.exists.return_value = True
        mocker.patch("subscribe.infrastructure.django_repository.Subscribe.objects.filter", return_value=mock_qs)
    ...
    def test_is_subscribed_false(self, mocker, repo):
        mock_qs = mocker.Mock()
    ...
    def test_add_calls_create(self, mocker, repo):
        mock_create = mocker.patch("subscribe.infrastructure.django_repository.Subscribe.objects.create")

        repo.add(1, 2)

        mock_create.assert_called_once_with(subscribe_from_id=1, subscribe_to_id=2)

    def test_delete_calls_filter_delete(self, mocker, repo):
        mock_qs = mocker.Mock()
        mocker.patch("subscribe.infrastructure.django_repository.Subscribe.objects.filter", return_value=mock_qs)
    ...
    def test_check_true(self, mocker, repo):
        mock_qs = mocker.Mock()
        mock_qs.exists.return_value = True
        mocker.patch("subscribe.infrastructure.django_repository.Subscribe.objects.filter", return_value=mock_qs)
    ...
    def test_check_false(self, mocker, repo):
        mock_qs = mocker.Mock()
        mock_qs.exists.return_value = False
        mocker.patch("subscribe.infrastructure.django_repository.Subscribe.objects.filter", return_value=mock_qs)
    ...
    def test_get_all_returns_list(self, mocker, repo):
    ...
    def test_get_all_empty(self, mocker, repo):
        mocker.patch(
            "subscribe.infrastructure.django_repository.Subscribe.objects.filter",
