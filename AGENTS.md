...
- Перед началом реализации назвать критерии успеха и проверки, которые подтвердят результат.
- При сообщении о баге сначала применять skill `diagnosing-bugs`, чтобы воспроизвести проблему и подтвердить её причину.
...
- До начала реализации зафиксировать текущий `HEAD` как baseline для review. После явного commit от пользователя, но до объявления изменения готовым, применять skill `code-review` к diff от baseline. Нерешённые findings исправлять, повторно проверять и review повторять от того же baseline.
...
- [2026-08-27] Проблема: команды `gh` зависали до отправки API-запроса, когда GitHub CLI не мог получить токен из GNOME Keyring → Решение: проверить `gh auth status`, восстановить доступ к keyring или повторно авторизовать `gh`, затем повторить команду в неинтерактивном режиме.
- [2026-08-27] Проблема: пустой вызов `apply_patch` без `patchText` завершился как aborted → Решение: перед вызовом передавать полностью сформированный patch envelope через обязательный параметр `patchText`.
- [2026-08-27] Проблема: `gh issue edit` однократно завершился transient-ошибкой GraphQL `EOF` → Решение: повторить идемпотентную команду и отдельно проверить фактическое состояние issue через `gh issue view`.
- [2026-08-27] Проблема: pre-commit hook `no-commit-to-branch` запретил коммит в `master` → Решение: не обходить hook, создать рабочую ветку и повторить pre-commit перед коммитом.
- [2026-08-27] Проблема: поиск selector в минифицированном CSS через файловый Grep превысил лимит JSON-записи из-за одной длинной строки → Решение: для точечной проверки generated CSS использовать `rg -o` с узким шаблоном, возвращающим только совпадение.
- [2026-08-27] Проблема: PID процесса Django autoreloader изменился между `ss` и чтением `/proc/<pid>/environ` → Решение: непосредственно перед чтением окружения повторно получить актуальный PID через `ss`, либо запускать диагностический сервер с `--noreload`.
- [2026-08-27] Проблема: авторизация при `TESTING=False` возвращала HTTP 500, потому что fail-hard cache alias `sessions` не мог подключиться к Redis на настроенном порту `16379` → Решение: перед ручной production-проверкой запускать compose-сервис командой `REDIS_PORT=16379 docker compose up -d moi-goroda-redis` и проверять его health.
- [2026-08-27] Проблема: `jq` прервал чтение `logs/log.log`, поскольку файл содержит строки разных форматов, а не только JSON → Решение: не обрабатывать весь смешанный лог как NDJSON; сначала выбирать JSON-строки либо использовать точечный текстовый поиск.
- [2026-08-27] Проблема: локальный Django-сервер с `DEBUG=False` и `--noreload` продолжал отдавать кешированную версию изменённого шаблона, поэтому мобильный браузер не видел правки после пересборки assets → Решение: после изменения Django-шаблонов перезапускать production-like сервер и проверять время запуска нового процесса перед ручной проверкой.
- [2026-08-28] Проблема: component-тест add-city modal искал rating radio по production-атрибуту `name`, отсутствующему в упрощённом test fixture → Решение: в тестах поведения modal использовать устойчивый fixture hook `#rating-container input`, если имя поля не является предметом проверки.
