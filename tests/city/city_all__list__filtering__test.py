...
def setup_db__filtering(client, django_user_model):
    ...
    for c in cities:
        city = City.objects.create(
            title=c[0], region=region, coordinate_width=1, coordinate_longitude=1
        )
...
def test__method_apply_filter_to_queryset__correct_value(
    setup_db__filtering, filter_value, expected_value
):
    """
    ...
    result = [
        city
        for city in mixin.apply_filter_to_queryset(queryset, filter_value).values_list(
...
def test__correct_order_of_filtered_cities_on_page(
    setup_db__filtering, client, filter_value, expected_value
):
    """
...
@pytest.mark.django_db
@pytest.mark.parametrize('sort_value', ['', 'name_down', 'name_up', 'date_down', 'date_up'])
def test__page_has_filter_buttons(setup_db__filtering, client, sort_value):
    ...
    button_filtering_by_magnet = buttons.find(
        'a',
        {
            'href': reverse('city-all-list')
            + (f'?filter=magnet&sort={sort_value}' if sort_value else '?filter=magnet')
        },
    )
    button_filtering_by_current_year = buttons.find(
        'a',
        {
            'href': reverse('city-all-list')
            + (f'?filter=current_year&sort={sort_value}' if sort_value else '?filter=current_year')
        },
    )
    button_filtering_by_last_year = buttons.find(
        'a',
        {
            'href': reverse('city-all-list')
            + (f'?filter=last_year&sort={sort_value}' if sort_value else '?filter=last_year')
        },
    )
