...
def setup_db__content_without_visited_regions__region_all(client, django_user_model):
    ...
    for num in range(1, 40):
        region = Region.objects.create(
            area=area, title=f'Регион {num}', type='область', iso3166=f'RU-RU-{num}'
        )
        City.objects.create(
...
def setup_db__content_with_visited_regions__region_all(client, django_user_model):
    ...
    for i in range(4, 6):
        City.objects.create(
            title=f'Город {i}', region=region, coordinate_width=1, coordinate_longitude=1
        )


@pytest.mark.django_db
def test__pagination__1st_page__auth_user(
    setup_db__content_without_visited_regions__region_all, client
):
...
@pytest.mark.django_db
def test__pagination__2nd_page__auth_user(
    setup_db__content_without_visited_regions__region_all, client
):
    client.login(username='username', password='password')
...
@pytest.mark.django_db
def test__pagination__3rd_page__auth_user(
    setup_db__content_without_visited_regions__region_all, client
):
    client.login(username='username', password='password')
...
@pytest.mark.django_db
def test__pagination__1st_page__guest(
    setup_db__content_without_visited_regions__region_all, client
):
    response = client.get(reverse('region-all-list'))
    source = BeautifulSoup(response.content.decode(), 'html.parser')
...
@pytest.mark.django_db
def test__pagination__2nd_page__guest(
    setup_db__content_without_visited_regions__region_all, client
):
    response = client.get(reverse('region-all-list') + '?page=2')
    source = BeautifulSoup(response.content.decode(), 'html.parser')
...
@pytest.mark.django_db
def test__pagination__3rd_page__guest(
    setup_db__content_without_visited_regions__region_all, client
):
    response = client.get(reverse('region-all-list') + '?page=3')
    source = BeautifulSoup(response.content.decode(), 'html.parser')
...
@pytest.mark.django_db
def test__without_visited_regions__1st_page__auth_user(
    setup_db__content_without_visited_regions__region_all, client
):
    client.login(username='username', password='password')
    response = client.get(reverse('region-all-list'))
    ...
    for num in range(1, 17):
        ...
        assert (
            card.find('div', {'id': f'section-qty_of_cities_{num}'}).find(
...
@pytest.mark.django_db
def test__without_visited_regions__2nd_page__auth_user(
    setup_db__content_without_visited_regions__region_all, client
):
    client.login(username='username', password='password')
    response = client.get(reverse('region-all-list') + '?page=2')
...
@pytest.mark.django_db
def test__without_visited_regions__3rd_page__auth_user(
    setup_db__content_without_visited_regions__region_all, client
):
    client.login(username='username', password='password')
    response = client.get(reverse('region-all-list') + '?page=3')
...
@pytest.mark.django_db
def test__without_visited_regions__1st_page__guest(
    setup_db__content_without_visited_regions__region_all, client
):
    response = client.get(reverse('region-all-list'))
    source = BeautifulSoup(response.content.decode(), 'html.parser')
    ...
    for num in range(1, 17):
        ...
        assert (
            'Всего городов: 1'
            in card.find('div', {'id': f'section-qty_of_cities_{num}'}).get_text()
        )
        assert (
            card.find('div', {'id': f'section-qty_of_cities_{num}'}).find(
                'div', {'id': f'progress_bar_{num}'}
...
@pytest.mark.django_db
def test__without_visited_regions__2nd_page__guest(
    setup_db__content_without_visited_regions__region_all, client
):
    response = client.get(reverse('region-all-list') + '?page=2')
    source = BeautifulSoup(response.content.decode(), 'html.parser')
...
@pytest.mark.django_db
def test__without_visited_regions__3rd_page__guest(
    setup_db__content_without_visited_regions__region_all, client
):
    response = client.get(reverse('region-all-list') + '?page=3')
    source = BeautifulSoup(response.content.decode(), 'html.parser')
...
@pytest.mark.django_db
def test__with_visited_regions__1st_page__auth_user(
    setup_db__content_with_visited_regions__region_all, client
):
    client.login(username='username', password='password')
    response = client.get(reverse('region-all-list'))
    source = BeautifulSoup(response.content.decode(), 'html.parser')
    ...
    assert page_header
    assert 'Список регионов России' in page_header.get_text()
    card = content.find('div', {'id': 'region_card_1'})
    assert 'Регион' in card.find('h4', {'id': 'section-region_title_1'}).get_text()
    assert 'Area 1' in card.find('div', {'id': 'section-area_title_1'}).get_text()
    assert '3 из 5' in card.find('div', {'id': 'section-qty_of_cities_1'}).get_text()
    assert card.find('div', {'id': 'section-qty_of_cities_1'}).find(
        'div', {'id': 'progress_bar_1', 'style': 'width: 60%', 'aria-valuenow': '60'}
    )
