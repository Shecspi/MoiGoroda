...
class SubscriptionPageRepository:
    ...
    def get_payments_for_user(self, user: User) -> list[PremiumPayment]:
        ...
        return list(
            PremiumPayment.objects.filter(user=user).select_related('plan').order_by('-created_at')
        )

    def get_active_subscription(self, user: User) -> PremiumSubscription | None:
    ...
    def get_payment_by_id_and_user(self, payment_id: str, user: User) -> PremiumPayment | None:
