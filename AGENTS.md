...
- Новый UI максимально собирать из компонентов daisyUI 5 с проектным префиксом `dui-`; Tailwind utilities использовать преимущественно для layout, адаптивности и отступов.
- Собственный компонент или CSS создавать только когда в daisyUI нет подходящего компонента либо проектируется уникальный визуальный артефакт. Причина исключения должна быть очевидна из контекста или краткого комментария.
- Существующую проектную UI library поддерживать только в уже использующем её интерфейсе; новые UI-элементы на ней не строить, поскольку библиотека постепенно деприкейтится.
