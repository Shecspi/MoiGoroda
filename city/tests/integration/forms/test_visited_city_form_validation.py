...
class TestCleanCityDuplicateValidation:
    ...
    def test_clean_city_allows_first_visit(self, setup_data: dict[str, Any]) -> None:
        ...
        form.cleaned_data = {'city': moscow, 'date_of_visit': date(2024, 1, 1)}

        result = form.clean_city()

        assert result == moscow  # Не должно быть ошибки

    def test_clean_city_forbids_duplicate_with_same_date(self, setup_data: dict[str, Any]) -> None:
    ...
    def test_clean_city_forbids_duplicate_without_date(self, setup_data: dict[str, Any]) -> None:
        """Дубликат без даты запрещён."""
        user = setup_data['user']
        moscow = setup_data['moscow']

        VisitedCity.objects.create(
            user=user, city=moscow, date_of_visit=None, rating=5, is_first_visit=True
        )
        ...
        form = VisitedCity_Create_Form(request=request)
        form.instance = VisitedCity()
        form.cleaned_data = {'city': moscow, 'date_of_visit': None}
    ...
    def test_clean_city_different_users_no_conflict(
        ...
        # User1 посещает
        VisitedCity.objects.create(
            user=user1, city=moscow, date_of_visit=date(2024, 1, 15), rating=5, is_first_visit=True
        )
...
class TestFormInitQuerysetLogic:
    ...
    def test_init_with_region_in_data_filters_cities(self, setup_data: dict[str, Any]) -> None:
    ...
    def test_init_with_initial_city_populates_queryset(self, setup_data: dict[str, Any]) -> None:
    ...
    def test_init_without_errors_empties_city_queryset(self, setup_data: dict[str, Any]) -> None:
