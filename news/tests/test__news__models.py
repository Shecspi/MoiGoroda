...
def test__meta(setup_db):
    ...
    assert News.objects.get(id=1).__str__() == 'Новость 1'
    assert News.objects.get(id=2).__str__() == 'Новость 2'
...
def test__fields(setup_db):
    ...
    assert News.objects.get(id=1)._meta.get_field('title').blank == False
    ...
    assert News.objects.get(id=1)._meta.get_field('content').blank == False
