...
def test_subscribe_success(client_logged, user, other_user):
...
@pytest.mark.django_db
def test_subscribe_invalid_json(client_logged):
    url = reverse('save_subscribe')
...
@pytest.mark.django_db
def test_subscribe_forbidden_other_user(client_logged, other_user):
    ShareSettings.objects.create(user=other_user, can_subscribe=True)
    url = reverse('save_subscribe')
...
@pytest.mark.django_db
def test_subscribe_user_not_exists(client_logged, user):
    url = reverse('save_subscribe')
...
@pytest.mark.django_db
def test_subscribe_not_allowed(client_logged, user, other_user):
...
@pytest.mark.django_db
def test_unsubscribe_success(client_logged, user, other_user):
    ShareSettings.objects.create(user=other_user, can_subscribe=True)
...
@pytest.mark.django_db
def test_delete_subscriber_success(client_logged, user, other_user):
...
@pytest.mark.django_db
def test_delete_subscriber_invalid_json(client_logged):
    url = reverse('delete_subscribe')
    response = client_logged.post(url, data='{invalid json', content_type='application/json')
    assert response.status_code == 400
    assert response.json()['status'] == 'error'
...
@pytest.mark.django_db
def test_delete_subscriber_user_not_exists(client_logged):
    url = reverse('delete_subscribe')
    data = {'user_id': 9999}
    response = client_logged.post(url, data=json.dumps(data), content_type='application/json')
    assert response.status_code == 403
...
@pytest.mark.django_db
def test_delete_subscriber_not_subscribed(client_logged, user, other_user):
    url = reverse('delete_subscribe')
    data = {'user_id': other_user.id}
    response = client_logged.post(url, data=json.dumps(data), content_type='application/json')
    assert response.status_code == 403
