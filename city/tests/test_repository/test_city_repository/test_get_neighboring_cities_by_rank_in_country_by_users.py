...
def test_neighboring_cities_by_users_returns_correct_list(mocker, repo):
    """
    Проверяет, что возвращается результат _get_cities_near_index по городам страны.
    """
    ...
    mock_filter.return_value.annotate.return_value.values.return_value.order_by.return_value = ranked_cities
    mock_get_cities_near_index = mocker.patch.object(repo, '_get_cities_near_index', return_value=ranked_cities[5:15])
    result = repo.get_neighboring_cities_by_rank_in_country_by_users(42)
...
def test_neighboring_cities_by_users_city_does_not_exist(mocker, repo):
    """
    ...
    result = repo.get_neighboring_cities_by_rank_in_country_by_users(123)
    assert result == []

def test_neighboring_cities_by_users_multiple_objects_returned(mocker, repo):
    """
...
def test_neighboring_cities_by_users_empty_ranked_list(mocker, repo):
    """
...
def test_neighboring_cities_by_users_orm_methods_called(mocker, repo):
    """
    ...
    repo.get_neighboring_cities_by_rank_in_country_by_users(42)
