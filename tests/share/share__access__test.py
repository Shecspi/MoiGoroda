...
def test__access__not_existing_user(
...
@pytest.mark.parametrize(
    'username, password, requested_user_id, status_code',
    ((None, None, 1, 404), ('username1', 'password', 1, 404), ('username2', 'password', 1, 404)),
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            302,
            'city_map',
        ),  # dashboard не доступен, должно быть перенаправление на city_map
        (
            (True, False, False, True),
            '',
            302,
            'region_map',
        ),  # dashboard не доступен, должно быть перенаправление на region_map
        (
            (True, True, False, False),
            'city_map',
            302,
            'dashboard',
        ),  # city_map не доступна, должно быть перенаправление на dashboard
        ...
        (
            ...
            302,
            'region_map',
        ),  # city_map не доступна, должно быть перенаправление на regino_map
        (
            (True, True, False, False),
            'region_map',
            302,
            'dashboard',
        ),  # region_map не доступна, должно быть перенаправление на dashboard
        (
            (True, False, True, False),
            'region_map',
            302,
            'city_map',
        ),  # region_map не доступна, должно быть перенаправление на city_map
        (
            (True, False, False, True),
            'region_map',
            200,
            None,
        ),  # Все ок, открываем страницу region_map
    ),
)
def test__access_has_no_permissions_with_db_entry(
    setup, client, settings, requested_page, status_code, redirect_page
):
    ...
    if redirect_page:
        if requested_page:
            response = client.get(
                reverse('share', kwargs={'pk': 1, 'requested_page': requested_page}), follow=True
            )
        else:
...
@pytest.mark.parametrize(
    ...
    (('', 200), ('city_map', 200), ('region_map', 200), ('sdfsdf', 404)),
)
